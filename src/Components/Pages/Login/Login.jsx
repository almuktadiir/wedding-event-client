...
const Login = () => {
    const { createLogin, handleGoogleLogin } = useContext(AuthContext);
    ...
    const handleLogin = e => {
        ...
        if(password.length < 6){
            return toast('password should be 6 characters or longer');
        }else if(!/^(?=.*[A-Z])(?=.*[\W_]).+$/.test(password)){
            return toast('Password must have at least one uppercase, lowecase and special characters');
        }
    ...
    return (
        <div className="relative flex w-96 flex-col rounded-xl bg-white bg-clip-border text-gray-700 shadow-md my-14 mx-auto">
            ...
            <div className="mb-4 text-center border-t-pink-500 bg-pink-50">
                <button onClick={handleGoogleSignIn} className="btn btn-outline border-pink-500 text-[#ee62a8ff] hover:bg-[#ee62a8ff] hover:border-0 my-6 ">
                    <FaGoogle></FaGoogle>
